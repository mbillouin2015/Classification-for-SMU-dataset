...
for i in tqdm(range(var.shape[0])):
    try:
        st.append(client.timeseries(var['net'][i],var['station'][i], '', var['chan'][i], var['datetime'][i], var['datetime'][i] + 10, filename=var['net'][i] + '.' + var['station'][i] + '.' + var['chan'][i] + '.'+ str(i) + '.sac', output='sacbb'))
    except:
